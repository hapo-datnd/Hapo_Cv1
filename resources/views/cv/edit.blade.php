...
@section('center-header')
    <div class="row m-0 justify-content-center align-content-center w-75">
        @csrf
        <input id="title" class="input-group-text m-auto w-50" value="{{$cv->title}}">
        <button type="button" id="btnSaveSubmit" class="btn-outline-success btn">Update</button>
    </div>
@endsection
@if(count($errors) > 0)
...
@section('content')
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(document).on('change','.input-group-file-edit',function () {
                let fileInput = $(this);
                let filePath = fileInput.val();
                let allowedExtensions = /(\.jpg|\.jpeg|\.png)$/i;
                if (!allowedExtensions.exec(filePath)) {
            ...
            $('#btnSaveSubmit').click(function(e){
                ...
                let contentSlideEdit = [];
                let idInputEdit = [];

                let numberSlideEdit = countGraphic("slick","get-content-slide-edit");
                for (let i =0; i < numberSlideEdit; i++) {
                    let idContentSlide = $('.get-content-slide-edit')[i].id;
                    contentSlideEdit[i] = ($('#'+idContentSlide).html());
                    formData.append(`content_slide_edit[${i}]`, contentSlideEdit[i]);
                    idInputEdit[i]  = $('.input-group-file-edit')[i].id;
                }

                for (let i =0; i < idInputEdit.length; i++) {
                    formData.append('image_slide_edit[]',$('#'+idInputEdit[i])[0].files[0]);
                }
                ...
                for (let i =0; i < numberSlide; i++) {
                    ...
                    idOfSlide[i] = Number(idContentSlide.substring(charactersOfLastSlideId));
                    formData.append(`slides[${i}][content]`, contentSlide[i]);
                    formData.append(`slides[${i}][id]`, idOfSlide[i])
                    idInput[i]  = $('.input-group-file')[i].id;
                    formData.append(`slides[${i}][image]`,$('#'+idInput[i])[0].files[0]);
                }
